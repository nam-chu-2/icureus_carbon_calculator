Qualtrics.SurveyEngine.addOnReady(function () {
  console.log(" =-= GHG Counterfactual script started =-=");
  ...
  const heatingTypeWeights = {
    ...
    // same as electric, varies by province
    wood: 0.001,
    unknown: 50.149,
    "": 50.149,
  };
...
// VH: I need to find a new source for these weights and update them.
...
*/
  ...
  AB: {
    ...
    diesel:  { truck: 0.266, car: 0.181, suv: 0.266 },
    hybrid:  { truck: 0.219, car: 0.144, suv: 0.219 },
    phev:    { truck: 0.133, car: 0.089, suv: 0.133 },
    battery: { truck: 0.094, car: 0.058, suv: 0.094 },
  },
  ...
  ON: {
    ...
    diesel:  { truck: 0.266, car: 0.181, suv: 0.266 },
    hybrid:  { truck: 0.219, car: 0.144, suv: 0.219 },
    phev:    { truck: 0.125, car: 0.083, suv: 0.125 },
    battery: { truck: 0.072, car: 0.045, suv: 0.072 },
  },
  ...
  /*
   * -----------------------------------------
   * Counterfactual Calculation Functions
   * -----------------------------------------
   */

  /**
   * EV_CF: Counterfactual vehicle emissions if the person drove a battery electric vehicle.
   * Uses the person's actual mileage, vehicle size, and passenger count,
   * but swaps fuel type to "battery".
   * If the person has no vehicle (mileage = 0), this returns 0.
   */
  function calculateEV_CF() {
    const vehicleSize = G_SurveySettings.vehicle.vehicleSize;
    let mileage = G_SurveySettings.vehicle.mileage;
    const passengerSize = G_SurveySettings.vehicle.passengers;
    ...
    const region = G_GetProvince;
    const regionWeights = vehicleTypeWeights[region];
    const fuelWeights = regionWeights["battery"];
    const factor = fuelWeights[vehicleSize] / 1000;
  ...
  /**
   * Gas_CF: Counterfactual vehicle emissions if the person drove a gas (petrol) vehicle.
   * Uses the person's actual mileage, vehicle size, and passenger count,
   * but swaps fuel type to "petrol".
   * If the person has no vehicle (mileage = 0), this returns 0.
   */
  function calculateGas_CF() {
    const vehicleSize = G_SurveySettings.vehicle.vehicleSize;
    let mileage = G_SurveySettings.vehicle.mileage;
    const passengerSize = G_SurveySettings.vehicle.passengers;
    ...
    const region = G_GetProvince;
    const regionWeights = vehicleTypeWeights[region];
    const fuelWeights = regionWeights["petrol"];
    const factor = fuelWeights[vehicleSize] / 1000;
  ...
  /**
   * Avg_Gas_CF: Counterfactual vehicle emissions if the person drove a gas (petrol)
   * vehicle the average distance of 17,000 km/year.
   * Designed for respondents without a vehicle: shows what emissions would be
   * if they drove a petrol car at the national average distance.
   * Uses the person's vehicle size (defaults to "car" if no vehicle) and 1 passenger.
   */
  function calculateAvgGas_CF() {
    const vehicleSize = G_SurveySettings.vehicle.vehicleSize; // defaults to "car" for non-drivers
    const mileage = 17000; // average annual distance in km
    const passengerSize = 1;
  ...
  /**
   * HeatPump_CF: Counterfactual heating emissions if the person switched to a heat pump.
   * Uses the person's actual house size, building standard, household size, and region,
   * but swaps the heating type and efficiency to "heatpump".
   */
  function calculateHeatPump_CF() {
    const region = G_GetProvince;
    const houseSize = G_SurveySettings.heating.houseSize;
    const buildingStandard = G_SurveySettings.heating.buildingStandard;
    const householdSize = G_SurveySettings.heating.householdSize;

    const heatingFactor = heatingTypeWeights["heatpump"][region];

    const heatingEmissionValue =
      ((houseSize / 10.7639) *
        buildingStandardWeight[region][buildingStandard] *
        (heatingFactor / heatingEfficiencyWeight["heatpump"]) / 1000) /
      householdSize;
  ...
  /**
   * Vegan_CF: Counterfactual diet emissions if the person were vegan.
   * Returns the vegan diet emission value regardless of the person's actual diet.
   */
  function calculateVegan_CF() {
    return Number(dietTypeWeights["vegan"].toFixed(1));
  }
 ...
 if (getCarAccess(qData.q1)) {
  ...
  /*
   * -----------------------------------------
   * Calculate and Set Embedded Data
   * -----------------------------------------
   */
  G_TotalEmissionsChartValue = calculateTotalEmissions();

  let flights = calculateFlightEmissions();
  Qualtrics.SurveyEngine.setEmbeddedData('Flights', flights.toFixed(1))
  let mobility = calculateVehicleEmissions();
  Qualtrics.SurveyEngine.setEmbeddedData('Mobility', mobility.toFixed(1))
  let diet = calculateDietEmissions();
  Qualtrics.SurveyEngine.setEmbeddedData('Diet', diet.toFixed(1))
  let heat = calculateHeatingEmissions();
  Qualtrics.SurveyEngine.setEmbeddedData('Heat', heat.toFixed(1))

  // Actual total emissions
  Qualtrics.SurveyEngine.setEmbeddedData(
    'GHG_Total',
    G_TotalEmissionsChartValue.toFixed(1)
  );

  // Counterfactual: EV - what if they drove a battery electric vehicle?
  let evCF = calculateEV_CF();
  Qualtrics.SurveyEngine.setEmbeddedData(
    'EV_CF',
    evCF.toFixed(1)
  );

  // Counterfactual: Gas - what if they drove a gas (petrol) vehicle?
  let gasCF = calculateGas_CF();
  Qualtrics.SurveyEngine.setEmbeddedData(
    'Gas_CF',
    gasCF.toFixed(1)
  );

  // Counterfactual: Avg Gas - what if they drove a gas vehicle 17,000 km/year?
  let avgGasCF = calculateAvgGas_CF();
  Qualtrics.SurveyEngine.setEmbeddedData(
    'Avg_Gas_CF',
    avgGasCF.toFixed(1)
  );

  // Counterfactual: Heat Pump - what if they switched to a heat pump?
  let heatPumpCF = calculateHeatPump_CF();
  Qualtrics.SurveyEngine.setEmbeddedData(
    'HeatPump_CF',
    heatPumpCF.toFixed(1)
  );

  // Counterfactual: Vegan - what if they were vegan?
  let veganCF = calculateVegan_CF();
  Qualtrics.SurveyEngine.setEmbeddedData(
    'Vegan_CF',
    veganCF.toFixed(1)
  );
  ...
  console.log(
    "*********Counterfactuals - EV_CF, Gas_CF, Avg_Gas_CF, HeatPump_CF, Vegan_CF: ",
    evCF, gasCF, avgGasCF, heatPumpCF, veganCF
  );

  /*
   * -----------------------------------------
   * JQuery Set Text Variable
   * -----------------------------------------
   */
  jQuery("#footprint").html(G_TotalEmissionsChartValue.toFixed(1).toString() + " t CO<sub>2</sub>");
