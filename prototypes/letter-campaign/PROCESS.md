...
## 9. One-screen sign step for Canada (2026-07-31, PI request)

The intermediate "Your MP: … → Continue to the letter" screen was removed for Canada: a successful lookup (typed FSA, or auto-run from the Qualtrics-piped `&fsa=` parameter) now lands directly on a single screen showing the MP card (photo, name, party, riding), a "Not your MP?" correction link (manual lookup / re-enter postal prefix), and the letter with the signature box. With the piped FSA, the respondent's entire journey is: click link → see MP + letter → type name → send. The US flow keeps its rep-result panel (its "continue" is an external redirect, not a letter).

## 10. US lookup via the team's ZIP3 spreadsheet (2026-07-31, PI request)

The 5 Calls API dependency was dropped entirely. The team compiled `part 2/spreadsheets/zip3_congress_contacts.csv` (one row per ZIP3 prefix: House rep names, parties, and contact pages), and `make_zip3_contacts.py` compiles it into `zip3-contacts.js` (887 ZIP3 entries, ~170 KB), which the page loads as a second static file. The US flow is now: `&zip3=` piped from the survey's existing embedded data (or typed — first 3 digits suffice) → table lookup → rep card → "Go to my representative's website" opens the member's contact page. ZIP3s spanning multiple districts (~74%) show the chooser; the ~25 special-use ZIP3s with no House rep fall back to the house.gov finder panel. Privacy improvement: the US lookup is fully client-side, so the ZIP never leaves the browser. Maintenance note: the spreadsheet must be kept current with rep turnover and the table regenerated before launch.
